...
int CSimulation::nDoBeachErosionOnCells(int const nCoast, int const nPoly, double const dTotTargetToErode)
{
   ...
   {
      LogStream << m_ulTimestep << ": " << ERR << "in nDoBeachErosionOnCells() for polygon " << nPoly << ", could not find the seaward end point of the up-coast profile (" << nUpCoastProfile << ") for depth of closure = " << m_dDepthOfClosure << ". Lengthen the coastline normals." << endl;
...
//          LogStream << WARN << "01 @@@@ while eroding polygon " << nPoly << " in DOWN-COAST direction, hit edge of grid at [" << nParProfEndX << "][" << nParProfEndY << "] for parallel profile from coast point " << nCoastPoint << " at [" << nCoastX << "][" << nCoastY << "]. Constraining this parallel profile at its seaward end" << endl;
      ...
      while (true)
      {
         ...
         if (nInlandOffset > 0)
         {
            ...
            {
//                LogStream << m_ulTimestep << ": reached end of up-coast profile " << nUpCoastProfile << " during down-coast ACTUAL erosion of unconsolidated sediment for coast " << nCoast << " polygon " << nPoly << " (nCoastPoint = " << nCoastPoint << " nInlandOffset = " << nInlandOffset << ")" << endl;
...
//                LogStream << "\tnPoly = " << nPoly << " doing DOWN-COAST, potential beach erosion = " << dElevDiff << " at [" << nX << "][" << nY << "] = {" << dGridCentroidXToExtCRSX(nX) << ", " <<  dGridCentroidYToExtCRSY(nY) << "} nCoastPoint = " << nCoastPoint << " nDistSeawardFromNewCoast = " << nDistSeawardFromNewCoast << " dThisElevNow = " << dThisElevNow << " Dean Elev = " << dVParProfileDeanElev[nDistSeawardFromNewCoast] << endl;
...
//       LogStream << "\tIn nDoBeachErosionOnCells() going DOWN-COAST, nPoly = " << nPoly << " nCoastPoint = " << nCoastPoint << " nInlandOffset = " << nInlandOffset << " dAllSedimentTargetPerProfile = " << dAllSedimentTargetPerProfile << " dSedToErodeOnThisProfile = " <<  dAllSedimentTargetPerProfile << " dSedToErodeOnThisPolygon = " << dSedToErodeOnThisPolygon << " dTotTargetToErode = " << dTotTargetToErode << endl;
   ...
   if (dSedToErodeOnThisPolygon > SEDIMENT_ELEV_TOLERANCE)
   {
      ...
      if (nIndex == INT_NODATA)
      {
         LogStream << m_ulTimestep << ": " << ERR << "in nDoBeachErosionOnCells() for polygon " << nPoly << ", could not find the seaward end point of the down-coast profile (" << nUpCoastProfile << ") for depth of closure = " << m_dDepthOfClosure << ". Lengthen the coastline normals." << endl;
...
//             LogStream << WARN << "05 @@@@ while eroding polygon " << nPoly << " in UP-COAST direction, hit edge of grid at [" << nParProfEndX << "][" << nParProfEndY << "] for parallel profile from coast point " << nCoastPoint << " at [" << nCoastX << "][" << nCoastY << "]. Constraining this parallel profile at its seaward end" << endl;
         ...
         while (true)
         {
            ...
            if (nInlandOffset > 0)
            {
               ...
               {
//                   LogStream << m_ulTimestep << ": reached end of down-coast profile " << nDownCoastProfile << " during up-coast ACTUAL erosion of unconsolidated sediment for coast " << nCoast << " polygon " << nPoly << " (nCoastPoint = " << nCoastPoint << " nInlandOffset = " << nInlandOffset << ")" << endl;
               ...
               {
                  // It isn't
//                   LogStream << m_ulTimestep << ": " << WARN << "reached edge of grid at [" << nXDownCoastThisStart << "][" << nYDownCoastThisStart << "] during UP-COAST erosion of unconsolidated sediment for coast " << nCoast << " polygon " << nPoly << " (nCoastPoint = " << nCoastPoint << " nInlandOffset = " << nInlandOffset << ")" << endl;
...
//                   if (m_ulTimestep == 16)
//                      LogStream << "\tnPoly = " << nPoly << " going UP-COAST, potential beach erosion = " << dElevDiff << " at [" << nX << "][" << nY << "] = {" << dGridCentroidXToExtCRSX(nX) << ", " <<  dGridCentroidYToExtCRSY(nY) << "} nCoastPoint = " << nCoastPoint << " nDistSeawardFromNewCoast = " << nDistSeawardFromNewCoast << " dThisElevNow = " << dThisElevNow << " Dean Elev = " << dVParProfileDeanElev[nDistSeawardFromNewCoast] << endl;
...
//    int nIndex =  pUpCoastProfile->nGetCellGivenDepth(m_pRasterGrid, m_dDepthOfClosure, &PtiUpCoastPartProfileSeawardEnd);
   ...
   if (nIndex == INT_NODATA)
   {
      LogStream << m_ulTimestep << ": " << ERR << "in nDoBeachDepositionOnCells() for polygon " << nPoly << ", could not find the seaward end point of the up-coast profile (" << nUpCoastProfile << ") for depth of closure = " << m_dDepthOfClosure << ". Lengthen the coastline normals." << endl;
...
//          nParProfLen;
      ...
      while (true)
      {
         ...
         if (nParProfLen > (pUpCoastProfile->nGetNumCellsInProfile()))
         {
            ...
            LogStream << m_ulTimestep << ": " << ERR << "reached seaward end of up-coast profile during DOWN-COAST deposition of unconsolidated sediment for coast " << nCoast << " polygon " << nPoly << " (nCoastPoint = " << nCoastPoint << " nSeawardOffset = " << nSeawardOffset << ")" << endl;
         ...
         // Safety check
         if (! bIsWithinGrid(nSeaEndX, nSeaEndY))
         {
//             LogStream << WARN << "09 @@@@ while doing DOWN-COAST deposition on polygon " << nPoly << ", hit edge of grid at [" << nSeaEndX << "][" << nSeaEndY << "] for parallel profile from coast point " << nCoastPoint << " at [" << nCoastX << "][" << nCoastY << "]. Constraining this parallel profile at its seaward end" << endl;
            ...
            KeepWithinGrid(nCoastX, nCoastY, nSeaEndX, nSeaEndY);
         ...
         // Solve for dA so that the existing elevations at the end of the parallel profile, and at the end of a Dean equilibrium profile on that part-normal, are the same
         double dParProfDeanLen = dGetDistanceBetween(&PtiVParProfile.front(), &PtiVParProfile.back());
         double const dPower = 2.0 / 3.0;
         double dParProfA = (dParProfStartElev - dParProfEndElev) /  pow(dParProfDeanLen, dPower);

         // For the parallel profile, calculate the Dean equilibrium profile of the unconsolidated sediment h(y) = A * y^(2/3) where h(y) is the distance below the highest point in the profile at a distance y from the landward start of the profile
         nParProfLen = PtiVParProfile.size();
         dVParProfileDeanElev.resize(nParProfLen, 0);

         // The elevation of the coast point in the Dean profile is the same as the elevation of the current coast point
         dVParProfileDeanElev[0] = m_pRasterGrid->m_Cell[nCoastX][nCoastY].dGetSedimentTopElev();

         double
            dInc = dParProfDeanLen / (nParProfLen-1),
            dDistFromParProfStart = dInc;

         for (int n = 1; n < nParProfLen; n++)
         {
            ...
            if (n <= (nSeawardOffset+1))
               dVParProfileDeanElev[n] = dParProfStartElev;
            else
            {
               double dDistBelowHighest = dParProfA * pow(dDistFromParProfStart, dPower);
               dVParProfileDeanElev[n] = dParProfStartElev - dDistBelowHighest;
            }
...
//                LogStream << WARN << "10 @@@@ while doing DOWN-COAST deposition on polygon " << nPoly << ", hit edge of grid at [" << nX << "][" << nY << "] for parallel profile from coast point " << nCoastPoint << " at [" << nCoastX << "][" << nCoastY << "]. Constraining this parallel profile at its seaward end" << endl;
            ...
            if (! m_pRasterGrid->m_Cell[nX][nY].bBeachErosionOrDepositionThisTimestep())
            {
               double
                  dTmpElev = m_pRasterGrid->m_Cell[nX][nY].dGetSedimentTopElev(),
                  dDiff = dVParProfileDeanElev[n] - dTmpElev;
...
//             LogStream << "\tnPoly = " << nPoly << " going DOWN-COAST, [" << nX << "][" << nY << "] nCoastPoint = " << nCoastPoint << " nSeawardFromCoast = " << nSeawardFromCoast << " dThisElevNow = " << dThisElevNow << " Dean Elev = " << dVParProfileDeanElev[nSeawardFromCoast] << endl;

            // Subtract the two elevations
//             assert(nSeawardFromCoast < dVParProfileDeanElev.size());
            double dElevDiff = dVParProfileDeanElev[nSeawardFromCoast] - dThisElevNow;
            if (dElevDiff > SEDIMENT_ELEV_TOLERANCE)
            {
               bool
...
//                   LogStream << "\tIn nDoWithinPolygonBeachRedistribution(), nPoly = " << nPoly << " going UP-COAST, potential beach erosion = " << -dElevDiff << " at [" << nX << "][" << nY << "] nCoastPoint = " << nCoastPoint << " nSeawardFromCoast = " << nSeawardFromCoast << " dThisElevNow = " << dThisElevNow << " Dean Elev = " << dVParProfileDeanElev[nSeawardFromCoast] << endl;
...
//       int nIndex = pDownCoastProfile->nGetCellGivenDepth(m_pRasterGrid, m_dDepthOfClosure, &PtiDownCoastPartProfileSeawardEnd);
      ...
      if (nIndex == INT_NODATA)
      {
         LogStream << m_ulTimestep << ": " << ERR << "in nDoBeachDepositionOnCells() for polygon " << nPoly << ", could not find the seaward end point of the down-coast profile (" << nUpCoastProfile << ") for depth of closure = " << m_dDepthOfClosure << ". Lengthen the coastline normals." << endl;
         return RTN_ERR_BAD_BEACH_EROSION_PROFILE;
      }
...
//             nParProfLen;
         ...
         while (true)
         {
            ...
            if (nParProfLen > (pDownCoastProfile->nGetNumCellsInProfile()))
            {
               ...
               LogStream << m_ulTimestep << ": " << ERR << "reached seaward end of down-coast profile during UP-COAST deposition of unconsolidated sediment for coast " << nCoast << " polygon " << nPoly << " (nCoastPoint = " << nCoastPoint << " nSeawardOffset = " << nSeawardOffset << ")" << endl;
...
//                LogStream << WARN << "14 @@@@ while constructing parallel profile for UP-COAST deposition on polygon " << nPoly << ", hit edge of grid at [" << nX << "][" << nY << "] for parallel profile from coast point " << nCoastPoint << " at [" << nCoastX << "][" << nCoastY << "]. Constraining this parallel profile at its seaward end" << endl;
      ...
      //          LogStream << "\tnPoly = " << nPoly << " going UP-COAST, [" << nX << "][" << nY << "] nCoastPoint = " << nCoastPoint << " nSeawardFromCoast = " << nSeawardFromCoast << " dThisElevNow = " << dThisElevNow << " Dean Elev = " << dVParProfileDeanElev[nSeawardFromCoast] << endl;
...
//          LogStream << "\tIn nDoWithinPolygonBeachRedistribution() going UP-COAST, nPoly = " << nPoly << " nCoastPoint = " << nCoastPoint << " nSeawardOffset = " << nSeawardOffset << " dAllSedimentTargetPerProfile = " << dAllSedimentTargetPerProfile << " dSandToDepositOnProf = " << dSandToDepositOnProf << " dCoarseToDepositOnProf = " << dCoarseToDepositOnProf << " dSandToDepositOnPoly = " << dSandToDepositOnPoly << " dCoarseToDepositOnPoly = " << dCoarseToDepositOnPoly << endl;
   ...
   LogStream << m_ulTimestep << ": polygon " << nPoly << " " << strMsg << " deposited = " << (dTotSandDeposited + dTotCoarseDeposited) << " target = " << dTotTargetToDeposit << " (sand = " << dTotSandDeposited << " sand remaining = " << dSandToDepositOnPoly << " coarse = " << dTotCoarseDeposited << " coarse remaining = " << dCoarseToDepositOnPoly << ")" << endl;
