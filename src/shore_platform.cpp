...
//       LogStream << m_ulTimestep << ": from profile " << nProfile << ", doing potential platform erosion " << (nDirection == DIRECTION_DOWNCOAST ? "down" : "up") << "-coast, dist from profile = " <<  nDistFromProfile << endl;
      ...
      if (dDepthOfBreaking == DBL_NODATA)
      {
         ...
         LogStream << m_ulTimestep << ": not in active zone, from profile " << nProfile << " when doing potential platform erosion " << (nDirection == DIRECTION_DOWNCOAST ? "down" : "up") << "-coast, dist from profile = " <<  nDistFromProfile << endl;
         continue;
      }
