...
int CSimulation::nAssignAllCoastalLandforms(void)
{
   ...
   for (int nCoast = 0; nCoast < static_cast<int>(m_VCoast.size()); nCoast++)
   {
      for (int j = 0; j < m_VCoast[nCoast].nGetCoastlineSize(); j++)
      {
         ...
         {
            // Should never happen
            LogStream << m_ulTimestep << ": SWL (" << m_dThisTimestepSWL << ") is in basement on cell [" << nX << "][" << nY << "] = {" << dGridCentroidXToExtCRSX(nX) << ", " << dGridCentroidYToExtCRSY(nY) << "}, cannot assign coastal landform for coastline " << nCoast << endl;
         ...
         else if (nLayer == ELEV_ABOVE_SEDIMENT_TOP)
         {
            ...
            LogStream << m_ulTimestep << ": SWL (" << m_dThisTimestepSWL << ") is above sediment-top elevation (" << m_pRasterGrid->m_Cell[nX][nY].dGetSedimentTopElev() << ") on cell [" << nX << "][" << nY << "] = {" << dGridCentroidXToExtCRSX(nX) << ", " << dGridCentroidYToExtCRSY(nY) << "}, cannot assign coastal landform for coastline " << nCoast << endl;
...
//                LogStream << m_ulTimestep << ": CLIFF CREATED [" << nX << "][" << nY << "] = {" << dGridCentroidXToExtCRSX(nX) << ", " << dGridCentroidYToExtCRSY(nY) << "}" << endl;
            }
            else
            {
...
//                LogStream << m_ulTimestep << ": DRIFT CREATED [" << nX << "][" << nY << "] = {" << dGridCentroidXToExtCRSX(nX) << ", " << dGridCentroidYToExtCRSY(nY) << "}" << endl;
            }
         }
...
//                   LogStream << m_ulTimestep << ": CLIFF RE-CREATED [" << nX << "][" << nY << "] = {" << dGridCentroidXToExtCRSX(nX) << ", " << dGridCentroidYToExtCRSY(nY) << "}" << endl;
               }
               else
               {
...
// 
//          LogStream << m_ulTimestep << ": coast cell " << j << " at [" << nX << "][" << nY << "] = {" << dGridCentroidXToExtCRSX(nX) << ", " << dGridCentroidYToExtCRSY(nY) << "} has landform category = ";
// 
//          int
...
int CSimulation::nLandformToGrid(int const nCoast, int const nPoint)
{
   ...
   if (nCategory == LF_CAT_CLIFF)
   {
      ...
      {
//         cout << m_ulTimestep << ": cell [" << nX << "][" << nY << "] before removing cliff, dGetVolEquivSedTopElev() = " << m_pRasterGrid->m_Cell[nX][nY].dGetVolEquivSedTopElev() << ", dGetSedimentTopElev() = " << m_pRasterGrid->m_Cell[nX][nY].dGetSedimentTopElev() << endl;
...
//         cout << m_ulTimestep << ": cell [" << nX << "][" << nY << "] after removing cliff, dGetVolEquivSedTopElev() = " << m_pRasterGrid->m_Cell[nX][nY].dGetVolEquivSedTopElev() << ", dGetSedimentTopElev() = " << m_pRasterGrid->m_Cell[nX][nY].dGetSedimentTopElev() << endl;
//         cout << m_ulTimestep << ": cell [" << nX << "][" << nY << "] is no longer a cliff" << endl << endl;
      }
      else
      {
...
//          LogStream << m_ulTimestep << ": STILL CLIFF [" << nX << "][" << nY << "] = {" << dGridCentroidXToExtCRSX(nX) << ", " << dGridCentroidYToExtCRSY(nY) << "} is still a cliff landform, dRemaining = " << dRemaining << endl;
      }
