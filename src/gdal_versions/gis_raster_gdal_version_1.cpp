 ...
 *
 * \file gis_raster.cpp
 * \brief These functions use GDAL to read and write raster GIS files in several formats. This version will build with GDAL version 1
...
bool CSimulation::bWriteRasterGISFloat(int const nDataItem, string const* strPlotTitle, int const nLayer)
{
   ...
   GDALDataset* pDataSet = pDriver->Create(strFilePathName.c_str(), m_nXGridMax, m_nYGridMax, 1, GDT_Float32, papszOptions);
   if (NULL == pDataSet)
   {
   ...
   for (int nY = 0; nY < m_nYGridMax; nY++)
   {
      for (int nX = 0; nX < m_nXGridMax; nX++)
      {
         switch (nDataItem)
         {
            ...
            case (PLOT_WAVE_HEIGHT):
            {
               if ((! m_pRasterGrid->m_Cell[nX][nY].bIsInundated()) || m_pRasterGrid->m_Cell[nX][nY].bIsInActiveZone())
                  dTmp = m_dMissingValue;
               else
...
bool CSimulation::bWriteRasterGISInt(int const nDataItem, string const* strPlotTitle, double const dElev)
{
   ...
   GDALDriver* pDriver = GetGDALDriverManager()->GetDriverByName(m_strRasterGISOutFormat.c_str());
   char** papszOptions = NULL;      // driver-specific options
   GDALDataset* pDataSet = pDriver->Create(strFilePathName.c_str(), m_nXGridMax, m_nYGridMax, 1, GDT_Int32, papszOptions);
   if (NULL == pDataSet)
   {
   ...
   for (int nY = 0; nY < m_nYGridMax; nY++)
   {
      for (int nX = 0; nX < m_nXGridMax; nX++)
      {
         switch (nDataItem)
         {
            ...
            case (PLOT_INUNDATION_MASK):
            {
               nTmp = m_pRasterGrid->m_Cell[nX][nY].bIsInundated();
               break;
            }
